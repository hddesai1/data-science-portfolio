...
def train_revenue_prediction_model(
  ...
  # Load features
  dataframe = session.table(features_table).to_pandas()
  ...
  features = dataframe.drop('REVENUE', axis = 1)
  ...
  classifier = GridSearchCV(pipeline, param_grid=parameteres, cv=number_of_folds)

  classifier.fit(train_features, train_target)
  train_r2_score = classifier.score(train_features, train_target)
  test_r2_score = classifier.score(test_features, test_target)
  ...
  if save_model:
    if train_r2_score >= train_accuracy_threshold and test_r2_score >= test_accuracy_threshold:
      ...
      model_file = os.path.join(model_output_dir, 'classifier.joblib')
...
def create_data_pipeline_ml_tasks(session,resume=False):
  print("Creating root/parent Snowflake Task: dataset pipeline")
  ...
  print("Creating child/dependent Snowflake Task: classifier training")
...
if __name__ == "__main__":
  ...
  if session:
    print("Executing dataset pipeline function to load and transform the dataset using Snowpark DataFrames...")
    if data_pipeline_feature_engineering(session) == "SUCCESS":
      # Register data pipelining function as a Stored Procedure so it can be run as a task
      print("Registering dataset pipeline function as a Stored Procedure so it can run as a task on Snowflake...")
      ...
      print("Registering classifier training function as a Stored Procedure so it can run as a task on Snowflake...")
      session.sproc.register(
      ...
      print("Executing Stored Procedure to training_data the classifier on Snowflake...")
      ...
      if json.loads(ml_training_result)["Model saved"] == "True":
        print("Creating Snowflake Tasks to run dataset pipeline and classifier training Snowpark stored procedures to run on Snowflake on a set schedule...")
