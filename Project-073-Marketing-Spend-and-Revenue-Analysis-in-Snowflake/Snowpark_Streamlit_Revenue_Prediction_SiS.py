...
def load():
  dataset = session.table("BUDGET_ALLOCATIONS_AND_ROI").unpivot("Budget", "Channel", ["SearchEngine", "SocialMedia", "Video", "Email"]).filter(col("MONTH") != "July")
  alloc, rois, last_alloc = dataset.drop("ROI"), dataset.drop(["CHANNEL", "BUDGET"]).distinct(), dataset.filter(col("MONTH") == "June")
  return dataset.to_pandas(), alloc.to_pandas(), rois.to_pandas(), last_alloc.to_pandas()
...
def chart(chart_data):
  base = alt.Chart(chart_data).encode(alt.features("MONTH", sort=list(calendar.month_name), title=None))
...
dataset, alloc, rois, last_alloc = load()
...
chart(pd.concat([dataset, july]).reset_index(drop=True).replace(channels_upper, channels))
...
if st.button("❄️ Save to Snowflake"):
  with st.spinner("Making snowflakes..."):
    dataframe = pd.DataFrame({"MONTH": ["July"], "SEARCHENGINE": [budgets[0]], "SOCIALMEDIA": [budgets[1]], "VIDEO": [budgets[2]], "EMAIL": [budgets[3]], "ROI": [pred]})
    session.write_pandas(dataframe, "BUDGET_ALLOCATIONS_AND_ROI")  
