...
def get_retriever(
    ...
    Args:
        embedding_model_id (str): Identifier for the embedding classifier to use.
        k (int, optional): Number of top results to return. Defaults to 3.
        device (str, optional): Device to run the embedding classifier on ('cpu' or 'cuda'). Defaults to "cpu".

    Returns:
        MongoDBAtlasParentDocumentRetriever: Configured retriever instance that can search
            documents in MongoDB Atlas using the specified embedding classifier.
    """
