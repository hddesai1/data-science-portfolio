...
dataframe = dataframe[dataframe['repair_status']!='Unknown']
# Create a new column with the product name
dataframe['product'] = dataframe['partner_product_category'].apply(lambda x : x.split('~')[-1].strip())

# Create combined dataframe with counts and percentage
comb_df_category = combine_data_cnt_percent(dataframe, col_base='product_category', col_add='repair_status', col_prod='product')
comb_df_product = combine_data_cnt_percent(dataframe, col_base='product', col_add='repair_status', col_prod='product'
                                           ).sort_values(by='total', ascending=False)

# Calculate metrics
total_repairs = dataframe.shape[0]
num_categories = dataframe['product_category'].nunique()
num_products = dataframe['product'].nunique()
max_fixed = dataframe['repair_status'].value_counts(normalize=True).max()


# Create app object=============================================================================
app = Dash(__name__, external_stylesheets=[dbc.themes.SPACELAB, "/dataset/styles.css"])
...
header = html.Div([
    html.Img(src='dataset/repairing-tools.png', style={'width': '48px', 'margin-left': '10px'}),
...
comb_df_category_all = combine_data_cnt_percent(
    dataframe, col_base='product_category',
...
drdw_category = dcc.Dropdown(
    ...
    options= dataframe['product_category'].unique(),    
...
drdw_product = dcc.Dropdown(
    ...
    options= dataframe['product'].unique(),    
...
app.layout = dbc.Container([
    ...
    dbc.Row([
        dbc.Col([
            html.H6("Created with 🤍 by Hrishikesh", style={'margin':'10px'}),              
...
def toggle_collapse(open_cat_clicks, open_prod_clicks, close_table_clicks, is_open):
    if ctx.triggered_id in ["open-cat-table", "open-prod-table"]:
        dataframe = comb_df_category if ctx.triggered_id == "open-cat-table" else comb_df_product
        return (            
            True if is_open else not is_open,       # Keep the collapse state open or toggle it to open
            dataframe.to_dict("records"),                  # Update rowData
            create_column_defs(dataframe, max_fixed)       # Update columnDefs
        )
...
def update_scatter_cat(selected_category):
    if selected_category is None:
        ...
        patch2['dataset'][0]['marker']['color'] = ['yellow' if i == selected_index else '#486C94' for i in range(len(df_category_for_scatter))] 
        patch2['dataset'][0]['marker']['line']['color'] = ['red' if i == selected_index else 'white' for i in range(len(df_category_for_scatter))] 
        patch2['dataset'][0]['marker']['size'] = [25 if i == selected_index 
...
def update_scatter_prod(selected_product):
    if selected_product is None:
        ...
        # Highlight the selected marker and increase its size
        patch['dataset'][0]['marker']['color'] = ['yellow' if i == selected_index else '#486C94' for i in range(len(df_product_for_scatter))] 
        patch['dataset'][0]['marker']['line']['color'] = ['red' if i == selected_index else 'white' for i in range(len(df_product_for_scatter))] 
        patch['dataset'][0]['marker']['size'] = [25 if i == selected_index 
