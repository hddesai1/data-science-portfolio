...
def combine_data_cnt_percent(dataframe, col_base, col_add, col_prod, norm='index'):
    # Calculate counts and normalized counts
    count = dataframe[col_base].value_counts()
    count_norm = dataframe[col_base].value_counts(normalize=True)
    ...
    if col_base != col_prod:
        comb_df['n_products'] = dataframe.groupby(col_base)[col_prod].nunique()
    
    # Calculate crosstabs for counts and normalized counts
    ct_cnt = pd.crosstab(dataframe[col_base], dataframe[col_add])
    ct_norm = pd.crosstab(dataframe[col_base], dataframe[col_add], normalize=norm)
...
def create_column_defs(dataframe, max_fixed):
    column_defs = []
